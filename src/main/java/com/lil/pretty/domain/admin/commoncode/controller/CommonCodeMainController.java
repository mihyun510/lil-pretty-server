...
public class CommonCodeMainController {
	...
	public  ResponseEntity<CommonResponse> getAllAdminCommonCodeItemsCodes(){
		try {
			List<CommonCode> items = commonCodeMainService.getAllAdminCommonCodeItemsCodes();
	...
	public  ResponseEntity<CommonResponse> getCommCodeItems(@PathVariable String grpNm){
		try {
			List<CommonCode>  items = commonCodeMainService.getAdminCommonCodeItemsCodes(grpNm);
			return ResponseEntity.ok(new CommonResponse(true, items,"Success"));
		}
		catch(Exception e) {	
			return ResponseEntity.status(401).body(new CommonResponse(false,null,"Failed"));
